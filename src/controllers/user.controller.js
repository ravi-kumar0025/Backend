...
const refreshAccessToken= asyncHandler (async (req,res) =>{ 
    ...
    try { 
        ...
        const { accessToken,refreshToken }=await generateRefeshAccessToken (user._id) 
...
const changePassword=asyncHandler (async (req,res) =>{ 
    ...
    const userdetected = await User.findById (req?.user?._id) 
    console.log(req.user)
    const checkPassword = await userdetected.isPasswordCorrect (oldPassword) 

    if (!checkPassword) 
        throw new ApiError (401,"oldpassword is incorrect") 

    userdetected.password=newPassword   //new password set
    await userdetected.save ({ validateBeforeSave: false })     //bcrpt function already written
...
const getUser=asyncHandler (async (req,res) =>{ 
    ...
    .status (200) 
    .json (
       await new ApiResponse(200,req.user," current user")
    ) 
 }) 
...
const updateTextdetails=asyncHandler (async (req,res) =>{ 
    ...
    .status (200) 
    .json (
        await new ApiResponse(200,req.user.fullName,"User details updated")
    ) 
...
const updateavatar=asyncHandler (async (req,res) =>{ 
    ...
    if (!isrefreshToken) 
        throw new ApiError (401,"please login to update the cover image")
    // console.log("CHECK CHECK ",isrefreshToken," CHECK CHECK") 
    ...
    .status (200) 
    .json (
        new ApiResponse(200,req.user?.avatar,"avatar updated")
    ) 
 }) 
...
const updateCoverImage=asyncHandler (async (req,res) =>{ 
    ...
    .status (200) 
    .json (
        new ApiResponse(200,req.user?.coverImage,"coverImage updated")
    ) 
 }) 
